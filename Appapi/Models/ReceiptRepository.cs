    ...
    public static class ReceiptRepository
    {
        ...
        public static IEnumerable<Receipt> GetReceivingBasis(Receipt Condition)
        {
            ...
            if (RBs != null)//若经过筛选后收货依据列表不为空
            {
                ...
                for (int i = RBs.Count - 1; i >= 0; i--)
                {
                    ...
                    RBs[i].NotReceiptQty = (decimal)RBs[i].NeedReceiptQty - (sum is DBNull || sum == null ? 0 : (decimal)sum); //剩余可收数量
        ...
        public static string ReceiveCommitWithNonQRCode(Receipt batInfo)
        {
            try
            {
                ...
                string jsonStr = " text1: '{0}', text2: '{12}', text3: '{1}', text4: '{2}', text5: '{3}', text6: '" + batInfo.Plant + "', text7: '{4}', text8: '{5}', text9: '{6}', text10: '{7}', text11: '{8}', text12: '{9}', text13: '', text14: '{10}', text15: '{11}', text16: '', text17: '', text18: '', text19: '', text20: '', text21: '', text22: '', text23: '', text24: '', text25: '', text26: '', text27: '', text28: '', text29: '', text30: '' ";
            ...
            try
            {
                ...
                else
                {
                    ...
                    else if (theBatch.TranType == "PUR-SUB")
                    {
                        ...
                        {
                            res = ErpAPI.CommonRepository.D0506_01(null, theBatch.JobNum, (int)theBatch.AssemblySeq, (decimal)AcceptInfo.ArrivedQty - (decimal)theBatch.OurFailedQty, theBatch.BatchNo, AcceptInfo.Warehouse, AcceptInfo.BinNum, theBatch.Company, theBatch.Plant);
        ...
        public static bool ForceComplete(Receipt IQCInfo)
        {
            ...
            string sql = @"update Receipt set Result = '" + IQCInfo.Result + "', SecondUserID = '" + HttpContext.Current.Session["UserId"].ToString() + "', IQCDate = '" + OpDate + "', IsAllCheck = {0},  InspectionQty = {1}, PassedQty = {2}, FailedQty = {3}, isComplete = 1, IQCRemark = '" + IQCInfo.IQCRemark + "' where ID = {4}";
        ...
        public static string PrintQR(Receipt info)
        {
            ...
            string sql = @"select 
                pr.plant
                from erp.PORel pr             
                where pr.Company = '" + info.Company + "'   and   pr.ponum = " + info.PoNum + "   and   pr.poline = " + info.PoLine + "  and pr.porelnum = " + info.PORelNum + " ";
            ...
            string jsonStr = " text1: '{0}', text2: '{12}', text3: '{1}', text4: '{2}', text5: '{3}', text6: '" + plant + "', text7: '{4}', text8: '{5}', text9: '{6}', text10: '{7}', text11: '{8}', text12: '{9}', text13: '', text14: '{10}', text15: '{11}', text16: '', text17: '', text18: '', text19: '', text20: '', text21: '', text22: '', text23: '', text24: '', text25: '', text26: '', text27: '', text28: '', text29: '', text30: '' ";
        ...
        public static DataTable GetShipDetailListByPreparationNum(string PreparationNum) //获取所有连续委外工序的描述
        {
            string sql = @"select * from PrePareInventory where Number = '" + PreparationNum + "'";
            DataTable dt = Common.SQLRepository.ExecuteQueryToDataTable(Common.SQLRepository.APP_strConn, sql);

            return dt;
        }

        public static bool IsPOPartNumAndQRPartNumEqual(int ID, string QR) //获取所有连续委外工序的描述
        {
            string sql = @"select * from PrePareInventory where id = " + ID + "";
            DataTable ShipDetaiLine = Common.SQLRepository.ExecuteQueryToDataTable(Common.SQLRepository.APP_strConn, sql);

            sql = @"select PartNum from erp.OrderHed oh left join erp.OrderDtl od on oh.OrderNum = od.OrderNum and oh.Company= od.Company where oh.PONum = '{0}' and od.poline ='{1}' and oh.Company='001'";
            sql = string.Format(sql, ShipDetaiLine.Rows[0]["PoNum"], ShipDetaiLine.Rows[0]["PoLine"]);

            string POPartNum = Common.SQLRepository.ExecuteScalarToObject(Common.SQLRepository.ERP_strConn, CommandType.Text, sql, null).ToString();

            string QRPartNum = QR.Split('~')[1];

            return POPartNum == QRPartNum ? true : false;
        }
