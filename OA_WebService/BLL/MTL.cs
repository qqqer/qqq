...
namespace OA_WebService
{
    public static class MTL
    {
        public static string  DMRDiscardHandler (string paraXML)
        {
            ...
            MtlReportRepository.InsertDiscardRecord((int)discardReview.MtlReportID, (decimal)discardReview.ReviewQty, discardReview.DR_DMRUnQualifiedReason, (int)theReport.DMRID, discardReview.DR_DMRWarehouseCode, discardReview.DR_DMRBinNum, discardReview.DR_TransformUserGroup, discardReview.DR_Responsibility, discardReview.DR_DMRUnQualifiedReasonRemark, CommonRepository.GetReasonDesc(discardReview.DR_DMRUnQualifiedReason), discardReview.DR_ResponsibilityRemark);

            sql = " update MtlReport set checkcounter = checkcounter - " + discardReview.ReviewQty + ",DMRUnQualifiedQty = ISNULL(DMRUnQualifiedQty,0) + " + discardReview.ReviewQty + "  where id = " + (discardReview.MtlReportID) + "";
